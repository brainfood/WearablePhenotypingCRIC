...
# 2020-10-19: This script is updated to work with output from Kubios v3.4.1

#Rows that I need from the hrv file:
# 9: start time
# 51: sample IDs for sampling times
# 52: time spans for each sampling
# 55: Beats total
# 56: Beats corrected
# 57: Beats corrected (%)  ('Artifacts corrected %' in earlier Kubios version)
# 58: Effective data length (s)
# 59: Effective data length (%)
# 64: sample IDs for all the results
# 66-68: Results Overview (every other column)
# 72-82: Time-domain results - statistical parameters (every other column)
# 84-87: Time-domain results - geometric parameters (every other column)
# 89: Frequency-Domain labels (alternating between "FFT spectrum" and "AR spectrum")
# 91-93: Peak frequences results
# 95-100: Absolute powers results
# 102-104: Relative powers results
# 106-107: Normalized powers results
# 108-109: Additional frequency domain metrics
# 110: EDR (every other column - FFT spectrum only)
# 112-149: Nonlinear Results (every other column)
...
#Usage: perl parse_kubios_hrv_file.kubios_v3.4.1.pl kubios_file.txt > formatted_results.txt
...
#Extract "Beats total", "Beats corrected", "Beats corrected (%)", 
#"Effective data length(s)", and "Effective data length (%)"
$line = <INFILE>;
$line = <INFILE>;
for(my $file_line = 0; $file_line < 5; $file_line++) {
	$line = <INFILE>;
	chomp($line);
	@line_data = split(",", $line);
...
#Progress to Results Overview
do {
    $line = <INFILE>;
} until($line =~ m/Results Overview/);
...
do {
    chomp($line);
    @line_data = split(",", $line);
    push(@metric_list, trim($line_data[0]) . "...Results_Overview");
    my @curr_metric = extract_column_data($line);
    push(@metric_data, \@curr_metric);
